        ...
        self._decodeData.call(self, d, function(data){
            ...
            let tradesEvt = {
                ...
                data:_.map(data.f, entry => {
                    let price = undefined !== entry.t ? entry.t : parseFloat(new BigJ(entry.Q).times(entry.P));
                    let orderType = 'sell';
                    if ('BUY' == entry.OT)
                    {
                        orderType = 'buy';
        ...
        switch (methodName)
        {
            ...
            case 'ub':
                _.forEach(data.A, (entry) => {
                    ...
                    this._processBalancesDelta(entry);
                })
                break;
        };
    }
    ...
    this._decodeData(d, function(data){
        ...
        */
        ...
        let tradesEvt = {
            ...
            data:_.map(data.f, entry => {
                if (entry) {
                    const rawPrice = new BigJ(entry.Q).times(entry.R);
        ...
        _.forEach(data.D, (entry) => {
            ...
            let evt = {
                ...
                data:{
                    ...
                    timestamp: this._parseUtcDateTime(entry.T)
                }
            }
...
_processOrdersDelta(d)
{
    this._decodeData(d, function(data){
        ...
        evt.data.targetPrice = parseFloat(new BigJ(evt.data.targetRate).times(evt.data.quantity));
...
//        h: null }
// }
_processBalancesDelta(d) {
      this._decodeData(d, (data) => {
        // an error occurred
        if (undefined === data)
        {
            return;
